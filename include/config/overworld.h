...
#define OW_SEASON_ENCOUNTERS            TRUE        // If TRUE, will allow the user to define and use different encounter tables based on the season.
#define OW_SEASON_DISABLE_FALLBACK      FALSE        // If TRUE, if the encounter table for a specific map and season is empty, the area will have no encounters instead of falling back to the default season.
#define OW_SEASON_FALLBACK              SEASON_SPRING // The season that encounter tables fall back to if the current season has no data or seasonal encounters are disabled.
...
#define OW_FLAG_NO_ENCOUNTER        FLAG_TEMP_5  // If this flag is set, wild encounters will be disabled.
...
// When this flag is set, speaker names will not be displayed in textboxes
#define OW_FLAG_SUPPRESS_SPEAKER_NAME 0
