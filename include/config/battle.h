...
#define B_SMART_WILD_AI_FLAG        0x20     // If this flag is set, wild Pokémon will become smart, with all AI flags enabled.
...
#define B_FLAG_TERA_ORB_CHARGED     0x22     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     0x23     // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.
#define B_FLAG_SLEEP_CLAUSE         0x24     // If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
#define B_FLAG_NO_WHITEOUT          0x25     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
#define B_VAR_STARTING_STATUS       0x40F7     // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active.
...
#define B_VAR_WILD_AI_FLAGS         0x40F8     // If not 0, you can use this var to add to default wild AI flags. IMPORTANT: NOT usable with flags above (1 << 15)
                                          // This var should never remain non-zero long enough for the player to save.
                                          // For better wild AI handling, edit GetWildAiFlags() in src/battle_ai_main.c

#define B_VAR_DIFFICULTY            0x40F9     // If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by the developer using Script_SetDifficulty AFTER NewGameInitData has run.
