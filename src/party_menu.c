...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    {
        bool32 hasFieldMove = FALSE;
        
        // Check if any Pokémon in party knows this field move
        ...
        // If no Pokémon knows the move, check if player has the HM in bag
        if (!hasFieldMove && HasHMInBag(j))
        {
            hasFieldMove = TRUE;
        }
