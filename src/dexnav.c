...
#if DEXNAV_ENABLED
STATIC_ASSERT(FLAG_DEXNAV_SEARCHING != 0, DNFlagSearching_Must_Not_Be_Zero);
STATIC_ASSERT(FLAG_DEXNAV_DETECTOR != 0, DNFlagDetectorMode_Must_Not_Be_Zero);
STATIC_ASSERT(VAR_DEXNAV_SPECIES != 0, DNVarSpecies_Must_Not_Be_Zero);
STATIC_ASSERT(VAR_DEXNAV_STEP_COUNTER != 0, DNVarStepCounter_Must_Not_Be_Zero);
#endif
...
bool8 TryStartDexNavSearch(void)
{
    ...
    if (FlagGet(FLAG_DEXNAV_SEARCHING) || (val & DEXNAV_MASK_SPECIES) == SPECIES_NONE)
        return FALSE;
...
static u8 GetEncounterLevelFromMapData(u16 species, enum EncounterType environment)
{
    ...
    case ENCOUNTER_TYPE_LAND:    // grass
        const struct WildPokemonInfo *landMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_LAND, &timeOfDay, NULL);
    ...
    case ENCOUNTER_TYPE_WATER:    //water
        const struct WildPokemonInfo *waterMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_WATER, &timeOfDay, NULL);
    ...
    case ENCOUNTER_TYPE_HIDDEN:
        const struct WildPokemonInfo *hiddenMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_HIDDEN, &timeOfDay, NULL);
...
static bool8 CapturedAllLandMons(u32 headerId)
{
    u16 i, species;
    int count = 0;
    const struct WildPokemonInfo *landMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_LAND, NULL, NULL);
...
static bool8 CapturedAllWaterMons(u32 headerId)
{
    ...
    u16 species;
    u8 count = 0;
    const struct WildPokemonInfo *waterMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_WATER, NULL, NULL);
...
static bool8 CapturedAllHiddenMons(u32 headerId)
{
    ...
    u16 species;
    u8 count = 0;
    const struct WildPokemonInfo *hiddenMonsInfo = GetWildPokemonInfo(headerId, WILD_AREA_HIDDEN, NULL, NULL);
...
static void DrawSpeciesIcons(void)
{
    ...
    for (i = 0; i < HIDDEN_WILD_COUNT; i++)
    {
        ...
        y = ROW_HIDDEN_ICON_Y;
        if (FlagGet(FLAG_DEXNAV_DETECTOR))
            TryDrawIconInSlot(species, x, y);
...
bool8 TryFindHiddenPokemon(void)
{
    ...
    if (DEXNAV_ENABLED == 0
            || !FlagGet(FLAG_DEXNAV_DETECTOR)
            || FlagGet(FLAG_DEXNAV_SEARCHING)
            || GetFlashLevel() > 0)
    {
        if (stepPtr != NULL)
            (*stepPtr) = 0;
