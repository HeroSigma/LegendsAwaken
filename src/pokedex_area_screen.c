...
static bool8 MapHasSpecies(u32 headerId, enum TimeOfDay timeOfDay, u16 species);
...
static void FindMapsWithMon(u16 species)
{
    ...
    {
        if (MapHasSpecies(i, gAreaTimeOfDay, species))
        {
...
static bool8 MapHasSpecies(u32 headerId, enum TimeOfDay timeOfDay, u16 species)
{
    ...
    if (MonListHasSpecies(GetWildPokemonInfoByTime(headerId, WILD_AREA_LAND, timeOfDay, NULL, NULL), species, LAND_WILD_COUNT))
        return TRUE;
    if (MonListHasSpecies(GetWildPokemonInfoByTime(headerId, WILD_AREA_WATER, timeOfDay, NULL, NULL), species, WATER_WILD_COUNT))
        return TRUE;
...
#ifdef BUGFIX
    if (MonListHasSpecies(GetWildPokemonInfoByTime(headerId, WILD_AREA_FISHING, timeOfDay, NULL, NULL), species, FISH_WILD_COUNT))
#else
    if (MonListHasSpecies(GetWildPokemonInfoByTime(headerId, WILD_AREA_FISHING, timeOfDay, NULL, NULL), species, LAND_WILD_COUNT))
#endif
        return TRUE;
    if (MonListHasSpecies(GetWildPokemonInfoByTime(headerId, WILD_AREA_ROCKS, timeOfDay, NULL, NULL), species, ROCK_WILD_COUNT))
        return TRUE;
    return FALSE;
}
